...
using namespace cv;
using namespace std;
void buildTrackDB_KNN(const char * videoFilePath, vector< list<simple_tube *> > & s_database, \
	const int max_area, const int obj_num, const float extend_factor, \
	...
	Mat frame_mat, bgimg_mog, bgimg_knn, fgmask_mog, fgmask_knn, pp;
	...
	CvMemStorage * storage = cvCreateMemStorage();
	CvSeq * first_contour = NULL;
	CTracker tracker(0.1,0.2, s_database, 30.0,10,100);
	...
	while (frame = cvQueryFrame(capture)) {
		...
		/*
		mog->apply(frame_mat, fgmask_mog, 0.005);//-1默认
		...
		*/
		...
		//cvWaitKey(30);
		IplImage * tmp = cvCloneImage(&(IplImage) fgmask_knn);  
		//cvCvtColor(frame, frame_gray, CV_BGR2GRAY); //帧图像彩色转灰度
		//getFgImage(frame, fgImg, bgImg, threshold); //帧差法获取前景图像
		//cvShowImage("fg", fgImg);
		//cvWaitKey();
		//cvCvtColor(fgImg, tmp, CV_BGR2GRAY); //帧图像彩色转灰度
											 //cvCopy(fgImg, tmp);
		//cvCanny(tmp, tmp, 50, 190, 3);
		//cvShowImage("fg", tmp);
		//cvWaitKey();
		Mat edges=fgmask_knn.clone();
		Canny(fgmask_knn, edges, 20, 190, 3);
		...
		find_num = contours.size();
																								   /**!!!Be careful with the param below!!!**/
		if (find_num > (obj_num * 3) || frame == NULL || frame_id <= skip_begin_frame) {
			frame_id += 1;
			cvClearMemStorage(storage);
			cvReleaseImage(&tmp);
			continue;
		...
		if(centers.size()>0)
		{
			tracker.Update(centers, rects, frame_id);
			...
			for(int i=0;i<tracker.tracks.size();i++)
			{
				if(tracker.tracks[i]->trace.size()>1)
				{
					...
					{
						line(frame_mat,tracker.tracks[i]->trace[j],tracker.tracks[i]->trace[j+1],Colors[tracker.tracks[i]->track_id%9],2,CV_AA);
					}
				}
			}
		}
...
*  判断时间重叠
*/
void refineDB(vector< list<simple_tube *> > & s_database, const char * dbFilePath ) {
	if (s_database.empty()) {
		printf("Fail: Database is empty, cannot refine!\n");
		return;
	}
	FILE* fp = fopen(dbFilePath, "wt");  
	for(int i = 0; i < s_database.size(); i++)
	{
		list<simple_tube *> list_tube = s_database[i];

		for (list<simple_tube *>::iterator iter = list_tube.begin(); iter != list_tube.end(); iter++)
		{
