...
// ---------------------------------------------------------------------------
CTracker::CTracker(float _dt, float _Accel_noise_mag, hash_map<int, tube_seq> & database, double _dist_thres, int _maximum_allowed_skipped_frames,int _max_trace_length)
{
	dt=_dt;
...
//
// ---------------------------------------------------------------------------
void CTracker::Update(vector<Point2d>& detections, vector<CvRect>& detect_rects, int n_frame, int thres_min_area)
{
	...
	if(tracks.size()==0)
	{
		...
		{
			if(detect_rects[i].height * detect_rects[i].width <= thres_min_area)
				continue;
	...
	if(not_assigned_detections.size()!=0)
	{
		...
		{
			if(detect_rects[not_assigned_detections[i]].height * detect_rects[not_assigned_detections[i]].width <= thres_min_area)
				continue;
